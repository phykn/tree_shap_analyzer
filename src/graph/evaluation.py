...
def plot_reg_evaluation(
    ...
    source = pd.DataFrame(data=dict(x=true, y=pred))
    base = alt.Chart(data=source, title=f"Target = {target}")

    fig_1 = base.mark_circle(size=60, color="#EA4A54"
    ).encode(
        x = alt.X(
            "x:Q", 
        ...
        tooltip = [
            alt.Tooltip("x", title="Ground Truth"),
            alt.Tooltip("y", title="Prediction")
        ]
    )

    fig_2 = base.mark_line(color="black", strokeDash=[5,2]
    ).encode(
        x = alt.X(
            "x:Q", 
            scale=alt.Scale(domain=(minimum, maximum))
